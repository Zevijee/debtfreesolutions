...
document.addEventListener('DOMContentLoaded', function() {
    ...
    defualtZipCode = document.getElementById('hidden-zip').value;

    for (let i = 0; i < document.getElementsByClassName('zip-code').length; i++) {
        const zipCodeElement = document.getElementsByClassName('zip-code')[i];
        if (zipCodeElement.id !== defualtZipCode) {
...
function configureDisplay() {
    ...
    for (let i = 0; i < zipCodeElements.length; i++) {
        ...
        fetch('/update_zip', {
            method: 'POST',
            body: JSON.stringify({zip_code: selectedZipCode}),
            headers: {
